...
export const updateMovie = async( movieID:number, data: MovieInput, coverPicName?:string) =>{
    ...
    if(data.title){
        ...
        const duplicateTitle = await db.movies.findFirst({where: { title: cleanTitle}});
        if(duplicateTitle){
            throw new ApiError(StatusCodes.CONFLICT, "Movie with this title already Exists")
        }
    }
