...
import { addBookingApi } from '../../services/api';
...
const BookingConfirmation = ({ 
  ...
  useEffect(() => {
    ...
    if (isOpen && !hasCalledApi.current && booking && booking.schedule) {
      ...
      const fetchData = async () => {
        const bookingSummary = {
          ...
          show_time: formatTime(booking.slot?.time),
          show_date: booking.schedule?.date?.fullDate,
          booked_seats: selectedSeats?.map(s => s.seat_name || s.name).join(', '),
          total_price: finalAmount,
          showtime_id : booking.showtimeId.showtimeId,
        };

        try {
          const response = await addBookingApi(bookingSummary);
          toast.success(response?.data?.message || "Booking Saved!");
        ...
        <div className="text-center">
          ...
          <div className="bg-white/5 rounded-2xl p-6 mb-8 space-y-4 text-left">
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className="text-white font-bold">{booking.schedule?.date?.fullDate}</span>
            </div>

            <div className="flex justify-between text-xs">
              <span className="text-gray-500">Seats</span>
              <span className="text-white font-bold text-right max-w-[150px]">
                {selectedSeats?.map(s => s.seat_name || s.name).join(', ')}
              </span>
            </div>
