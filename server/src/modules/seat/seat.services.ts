import {
  enum_Seats_seat_type,
  enum_ShowtimeSeats_status,
  Prisma,
} from "@prisma/client";
import db from "../../config/db.js";
import { ApiError } from "../../utils/apiError.js";
import { StatusCodes } from "http-status-codes";
import  type { SeatInput }  from "./seat.validator.js";
  ...
  for (let r = 1; r <= total_rows; r++) {
    ...
    for (let c = 1; c <= total_columns; c++) {
      seats.push({
        ...
        col_number: c,
        seat_type: is_vip
          ? enum_Seats_seat_type.vip
          : enum_Seats_seat_type.standard,
      });
    }
  }
...
export const bookSeats = async (userID: number, data: SeatInput) => {
  const bookedSeat = await db.showtimeSeats.updateMany({
    where: {
      showtime_seat_id: {
        in: data.seatIds.map((id) => Number(id)),
      },
      status: "available",
    },
    data: {
  ...
  return await db.showtimeSeats.findMany({
    where: {
      showtime_seat_id: { in: data.seatIds.map((id) => Number(id)) },
    },
    include: {
      Seats: true,
    },
...
export const releaseSeats = async (showtimeSeatID: number) => {
  ...
  if (bookedSeat.count === 0) {
    const seat = await db.showtimeSeats.findUnique({
  ...
  return await db.showtimeSeats.findUnique({
    where: { showtime_seat_id: showtimeSeatID },
  });
};
